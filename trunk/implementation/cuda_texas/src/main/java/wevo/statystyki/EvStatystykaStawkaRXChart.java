...
public class EvStatystykaStawkaRXChart {
  ...
  public static JFreeChart createJFreeChart(final EvStatistic[] stats) {
    final JFreeChart chart =
        ChartFactory.createXYLineChart("Wysokosc stawki za karty w rundzie x (srednia populacji)", "Iteration",
  ...
  public static XYDataset getDataSet(final EvStatistic[] stats) {
    ...
    if (stats != null) {
      ...
      for (EvStatistic stat : stats) {
        float[] genes_value = ((EvStatystykaStawkiRXStatistic) stat).statystykiStawki;
