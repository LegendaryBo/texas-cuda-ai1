...
public class EvStatystykiWygranychIPrzegranychBestChart {
  ...
  public static JFreeChart createJFreeChart(final EvStatistic[] stats,
    ...
    if (ktoraCzesc)
      chart = ChartFactory.createXYLineChart("Liczba wygranych/przegranych/spasowanych rozdan (najlepszy os.)", "Iteracja",
            "Liczba rozdan", getDataSet(stats, ktoraCzesc),
            PlotOrientation.VERTICAL, true, false, false);
    else
      chart = ChartFactory.createXYLineChart("Srednia kwota przegrana/wygrana  (najlepszy os.)", "Iteracja",
  ...
  public static XYDataset getDataSet(final EvStatistic[] stats, boolean ktoraCzesc) {
    ...
    if (stats != null) {
      ...
      for (EvStatistic stat : stats) {
        ...
        if (ktoraCzesc) {
          genes_value = ((EvStatystykiWygranychIPrzegranychStatistic) stat).wynikiPartiBest;

          if (series == null) {
            series = new XYSeries[3];
            series[0] = new XYSeries("Przegranych (najlepszy os.)");
            dataset.addSeries(series[0]);
            series[1] = new XYSeries("Wygranych (najlepszy os.)");
            dataset.addSeries(series[1]);
            series[2] = new XYSeries("Passow (najlepszy os.)");
            dataset.addSeries(series[2]);            
          } 
        }
        else { 
          genes_value = ((EvStatystykiWygranychIPrzegranychStatistic) stat).srednieRezultatyBest;

          
          if (series == null) {
            series = new XYSeries[2];
            series[0] = new XYSeries("Srednia przegrana (najlepszy os.)");
            dataset.addSeries(series[0]);
            series[1] = new XYSeries("Srednia wygrana (najlepszy os.)");
            dataset.addSeries(series[1]);
