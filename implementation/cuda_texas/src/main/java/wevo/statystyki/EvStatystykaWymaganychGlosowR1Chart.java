...
public class EvStatystykaWymaganychGlosowR1Chart {
  ...
  public static JFreeChart createJFreeChart(final EvStatistic[] stats) {
    final JFreeChart chart =
        ChartFactory.createXYLineChart("Statystyki genow odpowiedzialnych za wziecie udzilu w grze (srednia populajci)", "Iteration",
  ...
  public static XYDataset getDataSet(final EvStatistic[] stats) {
    ...
    if (stats != null) {
      ...
      for (EvStatistic stat : stats) {
        float[] genes_value = ((EvStatystykaWymaganychGlosowR1Statistic) stat).wymaganychGlosow;
