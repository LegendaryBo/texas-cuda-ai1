...
public class EvStatystykiLicznikPassowBestChart {
  ...
  public static JFreeChart createJFreeChart(final EvStatistic[] stats) {
    final JFreeChart chart =
      ChartFactory.createXYLineChart("Rundy, w których najlepszy osobnik pasowal", "Iteracja",
  ...
  public static XYDataset getDataSet(final EvStatistic[] stats) {
    ...
    if (stats != null) {
      ...
      for (EvStatistic stat : stats) {
        int[] genes_value = null;
        
    
          genes_value = ((EvStatystykiLicznikPassowStatistic) stat).licznikPassowNajlepszego;
