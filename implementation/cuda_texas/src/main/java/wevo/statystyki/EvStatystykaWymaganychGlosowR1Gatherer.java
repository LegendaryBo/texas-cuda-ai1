...
public class EvStatystykaWymaganychGlosowR1Gatherer extends EvGatherer {
  ...
  public EvStatystykaWymaganychGlosowR1Gatherer(EvPersistentSimpleStorage storage3) {
    setStorage(storage3);
  }
  ...
  public EvStatistic generate(EvPopulation population) {
    ...
    for (int i=0; i < population.size(); i++) {
      ...
      dane[0] += kodGrayaWymaganych.getWartoscKoduGraya(ind);      
    ...
    daneBest[0] = kodGrayaWymaganych.getWartoscKoduGraya(best);      
    
    if (best.getGene(GeneratorRegul.indeksyGenowNaWejscie[0]) == 1)
      daneBest[1] = kodGrayaPara.getWartoscKoduGraya(best);    
    
    if (best.getGene(GeneratorRegul.indeksyGenowNaWejscie[1]) == 1)
      daneBest[2] = kodGrayaKolor.getWartoscKoduGraya(best);     
    
    if (best.getGene(GeneratorRegul.indeksyGenowNaWejscie[2]) == 1)
      daneBest[3] = kodGrayaOgrStawki.getWartoscKoduGraya(best);        
  
    if (best.getGene(GeneratorRegul.indeksyGenowNaWejscie[2]) == 1)
      daneBest[4] = kodGrayaOgrStawkiOgraniczenie.getWartoscKoduGraya(best);          
    
    if (best.getGene(GeneratorRegul.indeksyGenowNaWejscie[3]) == 1)
      daneBest[5] = kodGrayaWysokieKarty.getWartoscKoduGraya(best);        
 
    if (best.getGene(GeneratorRegul.indeksyGenowNaWejscie[4]) == 1)
      daneBest[6] = kodGrayaBardzoWysokieKarty.getWartoscKoduGraya(best);     
    
    iteration++;
    
    return new EvStatystykaWymaganychGlosowR1Statistic(iteration, dane, daneBest);
  }  
